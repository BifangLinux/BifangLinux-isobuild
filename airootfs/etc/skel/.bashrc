...
_set_my_PS1() {
    ...
    if [ "$(whoami)" = "liveuser" ] ; then
        local iso_version="$(grep ^VERSION= /usr/lib/bifanglinux-release 2>/dev/null | cut -d '=' -f 2)"
...
ShowInstallerIsoInfo() {
    local file=/usr/lib/bifanglinux-release
    if [ -r $file ] ; then
        cat $file
    else
...
_CheckArchNews() {
    local conf=/etc/bos-update-notifier.conf
